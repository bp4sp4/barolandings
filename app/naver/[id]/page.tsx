...
export default function NaverMaterialRedirect() {
  ...
  useEffect(() => {
    ...
    if (materialId) {
      ...
      router.replace(
        `/?utm_source=naver&utm_medium=search&utm_campaign=material_${materialId}&material_id=${materialId}`
      );
    } else {
      // materialId가 없으면 일반 네이버 페이지로 리다이렉트
      router.replace(
        "/?utm_source=naver&utm_medium=search&utm_campaign=recruitment"
      );
    }
  }, [router, params]);
